...
While the PostgreSQL adapter is pending, the controller injects a `MockLogRepo` placeholder that satisfies the interface structurally but silently discards all log data. You can see this in `src/infrastructure/controllers/pedido_local_controller.py`:
...
`MockLogRepo` accepts all calls without raising errors, which keeps the application functional during development. No data is persisted. **Do not deploy this to a production environment** — every invoice submission will be untracked and unrecoverable from the application's own records.
